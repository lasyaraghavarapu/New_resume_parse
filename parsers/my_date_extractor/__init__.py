...
def generate_patterns():
    ...
    patterns['punctuation_nocomma'] = u"(?: |-|\.|\s?\/\s?)"
    ...
    patterns['ym'] = u"(?<![\dA-Za-z])" + u"(?P<ym>" + patterns['year'].replace("year", "year_ym") + patterns[
        'punctuation'].replace("punctuation", "punctuation_ym") + patterns['month'].replace("month", "month_ym") + u")" + "(?<![^\d]\d{4})" + u"(?!-\d{1,2}-\d{1,2})(?![\dABCDEFGHIJKLMNOPQRSUVWXYZabcdefghijklmnopqrsuvwxyz])" + "(?<!" + \
                      patterns['nots'] + ")" + "(?<!" + patterns['times'] + ")"
    ...
    patterns['date'] = date = u"(?P<date>"  + patterns['my'] + "|" + patterns['ym'] + "|"+ patterns['y'] + u")"# + "|" + patterns['month'] + u")"
...
def extract_dates(text, sorting=None, debug=False, default_hour=0, default_minute=0, default_second=0, return_precision=False):
    ...
    for match in re.finditer(re.compile(patterns['date'], flags), text):
        ...
        if python_version == 2:
            match = dict((k.split("_")[0], num(v)) for k, v in match.groupdict().iteritems() if num(v) != None)
        elif python_version == 3:
            match = dict((k.split("_")[0], num(v)) for k, v in match.groupdict().items() if num(v) != None)
